...
class PromotionHistoryController extends Controller implements HasMiddleware
{
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.promotions.history');
    }
    ...
    public function rollback(int $id)
    {
        try {
            ...
            return response()->json([
                'status' => 'success',
                'message' => __('admin.promotions.messages.success.rollback'),
            ]);
        } catch (\Exception $ex) {
            return response()->json([
                'status' => 'error',
                'message' => $ex->getMessage() ?: __('admin.promotions.messages.failed.rollback'),
            ], 500);
        }
    }
}
