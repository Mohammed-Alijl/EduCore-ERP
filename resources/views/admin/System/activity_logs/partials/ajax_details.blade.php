...
<div class="log-details-content">
        ...
        <div class="col-md-6 mb-3">
            <div class="p-3 bg-light rounded">
                <h6 class="text-uppercase tx-11 text-muted font-weight-bold mb-2">
                    ...
                    {{ trans('admin.System.activity_logs.fields.subject') }}
                </h6>
                @if ($log->subject)
                    <p class="mb-0">
                ...
                @else
                    <p class="mb-0 text-muted">{{ trans('admin.System.activity_logs.no_subject') }}</p>
                @endif
            </div>
        </div>
        ...
        <div class="col-md-6 mb-3">
            <div class="p-3 bg-light rounded">
                <h6 class="text-uppercase tx-11 text-muted font-weight-bold mb-2">
                    ...
                    {{ trans('admin.System.activity_logs.fields.causer') }}
                </h6>
                @if ($log->causer)
                    <p class="mb-0">
                ...
                @else
                    <p class="mb-0 text-muted">{{ trans('admin.System.activity_logs.system_action') }}</p>
                @endif
            </div>
        </div>
    </div>
    ...
    @if ($log->batch_uuid)
        <div class="mb-3 p-3 bg-light rounded">
            <h6 class="text-uppercase tx-11 text-muted font-weight-bold mb-2">
                ...
                {{ trans('admin.System.activity_logs.fields.batch_uuid') }}
            </h6>
    ...
    @if ($log->properties && $log->properties->count())
        <div class="mt-3">
            <h6 class="text-uppercase tx-11 text-muted font-weight-bold mb-3">
                ...
                {{ trans('admin.System.activity_logs.properties_title') }}
            </h6>
            ...
            <div class="table-responsive">
                <table class="table table-sm table-bordered mb-0">
                    <thead class="thead-light">
                        <tr>
                            <th class="wd-25p">{{ trans('admin.System.activity_logs.fields.attribute') }}</th>
                            @if ($log->event === 'updated' && isset($log->properties['old']))
                                <th class="wd-35p">{{ trans('admin.System.activity_logs.fields.old_value') }}</th>
                                <th class="wd-35p">{{ trans('admin.System.activity_logs.fields.new_value') }}</th>
                            @else
                                <th>{{ trans('admin.System.activity_logs.fields.value') }}</th>
                            @endif
                        </tr>
                    </thead>
                    <tbody>
    ...
    @else
        <div class="text-center text-muted py-3 bg-light rounded mt-3">
            ...
            {{ trans('admin.System.activity_logs.no_properties') }}
        </div>
    @endif

    {{-- View Full Details Link --}}
    <div class="mt-4 text-center">
        <a href="{{ route('admin.System.activity_logs.show', $log->id) }}" class="btn btn-outline-primary">
            <i class="las la-external-link-alt mr-1"></i>
            {{ trans('admin.System.activity_logs.view_full_details') }}
        </a>
    </div>
</div>
