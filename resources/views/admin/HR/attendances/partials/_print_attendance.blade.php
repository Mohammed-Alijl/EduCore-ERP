...
<head>
    ...
    <title>{{ trans('admin.HR.attendances.print_title') ?? 'Attendance Report' }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="print-header">
        ...
        <p>{{ trans('admin.HR.attendances.print_title') ?? 'Attendance Report' }}</p>
    </div>

    <div class="print-info">
        <div class="print-info-item">
            <div class="print-info-label">{{ trans('admin.HR.attendances.attendance_date') }}:</div>
            <div class="print-info-value">{{ request('attendance_date') }}</div>
        </div>
        <div class="print-info-item">
            <div class="print-info-label">{{ trans('admin.HR.attendances.total_students') ?? 'Total Students' }}:</div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="min-width: 200px;">{{ trans('admin.HR.attendances.student_details') }}</th>
                <th style="min-width: 150px;">{{ trans('admin.HR.attendances.attendance_status') }}</th>
        ...
        <tbody>
            @forelse($students as $student)
                @php
                    ...
                    $statusLabels = [
                        1 => trans('admin.HR.attendances.present'),
                        2 => trans('admin.HR.attendances.absent'),
                        3 => trans('admin.HR.attendances.late'),
                        4 => trans('admin.HR.attendances.excused'),
                    ];
                    ...
                    $statusLabel = $statusLabels[$currentStatus] ?? trans('admin.HR.attendances.present');
            ...
            @empty
                <tr>
                    <td colspan="4" style="text-align: center; padding: 30px;">
                        {{ trans('admin.HR.attendances.no_students') ?? 'No students found' }}
                    </td>
                </tr>
            @endforelse
        </tbody>
    ...
    <div class="print-footer">
        <p>{{ trans('admin.HR.attendances.print_footer') ?? 'This is an automatically generated report' }}</p>
