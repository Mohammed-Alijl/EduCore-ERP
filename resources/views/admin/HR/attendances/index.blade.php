    ...
    <div class="breadcrumb-header justify-content-between attendance-page">
        <div class="my-auto">
            <div class="d-flex align-items-center">
                ...
                <div>
                    <h4 class="content-title mb-0 my-auto font-weight-bold">{{ trans('admin.HR.attendances.title') }}</h4>
                    <span class="text-muted mt-1 tx-13 d-block">{{ trans('admin.HR.attendances.subtitle') }}</span>
                </div>
            </div>
        </div>
        @can('print_attendances')
            <div class="d-flex align-items-center gap-2">
                <button type="button" class="btn btn-info btn-sm" id="btn_print_section_attendance">
                    ...
                    {{ trans('admin.HR.attendances.print_section') ?? 'Print Section' }}
                </button>
            </div>
        @endcan
    </div>
                        ...
                        <div class="row">
                            <div class="col-lg col-md-6 mb-3">
                                <label class="filter-label">
                                    <i class="las la-calendar"></i> {{ trans('admin.HR.attendances.attendance_date') }}
                                </label>
                            ...
                            <div class="col-lg col-md-6 mb-3">
                                <label class="filter-label">
                                    <i class="las la-calendar-check"></i> {{ trans('admin.HR.attendances.academic_year') ?? 'Academic Year' }}
                                </label>
                                <select class="form-control form-control-modern select2" id="filter_academic_year" name="academic_year_id">
                                    <option value="">{{ trans('admin.HR.attendances.select_academic_year') ?? 'Select Academic Year' }}</option>
                            ...
                            <div class="col-lg col-md-4 mb-3">
                                <label class="filter-label">
                                    <i class="las la-layer-group"></i> {{ trans('admin.HR.attendances.filter_grade') }}
                                </label>
                                <select class="form-control form-control-modern select2" id="filter_grade" name="grade_id">
                                    <option value="">{{ trans('admin.HR.attendances.select_grade') }}</option>
                            ...
                            <div class="col-lg col-md-4 mb-3">
                                <label class="filter-label">
                                    <i class="las la-chalkboard"></i> {{ trans('admin.HR.attendances.filter_classroom') }}
                                </label>
                                <select class="form-control form-control-modern select2" id="filter_classroom"
                                    name="classroom_id" disabled>
                                    <option value="">{{ trans('admin.HR.attendances.select_classroom') }}</option>
                                </select>
                            </div>

                            <div class="col-lg col-md-4 mb-3">
                                <label class="filter-label">
                                    <i class="las la-users"></i> {{ trans('admin.HR.attendances.filter_section') }}
                                </label>
                                <select class="form-control form-control-modern select2" id="filter_section"
                                    name="section_id" disabled>
                                    <option value="">{{ trans('admin.HR.attendances.select_section') }}</option>
                                </select>
                            </div>
                        </div>
                        ...
                        @can('view_attendances')
                            <div class="d-flex justify-content-end mt-2">
                                <button type="button" class="btn load-students-btn" id="btn_load_students">
                                    ...
                                    {{ trans('admin.HR.attendances.load_students') }}
                                </button>
                            </div>
                        @endcan
                    </div>
        ...
        $(document).ready(function() {
            ...
            class AttendanceManager {
                ...
                initElements() {
                    ...
                    this.routes = {
                        classrooms: "{{ route('admin.Academic.classrooms.by-grade') }}",
                        sections: "{{ route('admin.Academic.sections.by-classroom') }}",
                        students: "{{ route('admin.HR.attendances.students') }}",
                        store: "{{ route('admin.HR.attendances.store') }}",
                        print: "{{ route('admin.HR.attendances.print-section') }}"
                    };

                    this.trans = {
                        select: "-- {{ trans('admin.global.select') }} --",
                        loading: '<i class="las la-spinner la-spin mr-1"></i> {{ trans('admin.HR.attendances.loading') }}',
                        saving: '<span class="spinner-border spinner-border-sm mr-2"></span> {{ trans('admin.HR.attendances.saving') }}',
                        ok: "{{ trans('admin.global.ok') }}",
                        warning: "{{ trans('admin.HR.attendances.messages.failed.warning') }}",
                        errorTitle: "{{ trans('admin.HR.attendances.messages.failed.error') }}",
                        success: "{{ trans('admin.HR.attendances.messages.success.add') }}",
                        warningSelect: "{{ trans('admin.HR.attendances.messages.warning_select') }}",
                        errorFetch: "{{ trans('admin.HR.attendances.messages.error_fetch') }}",
                        errorSave: "{{ trans('admin.HR.attendances.messages.error_save') }}",
                        errorPrint: "{{ trans('admin.HR.attendances.messages.error_print') }}"
                    };
                }
