    ...
    <div class="teacher-actions-container">
        @can('edit_employees')
                ...
                if ($row->attachments->count() > 0) {
                    foreach ($row->attachments as $attachment) {
                        ...
                        $attachmentConfigs[] = [
                            'caption' => $fileName,
                            'type' => $type,
                            'url' => route('admin.Users.employees.attachments.destroy', $attachment->id),
                            'key' => $attachment->id,
                        ];
                    }
                }
            ...
            <a class="btn btn-teacher-edit btn-sm edit-btn" href="#" data-toggle="modal" data-target="#editEmployeeModal"
               data-url="{{ route('admin.Users.employees.update', $row->id) }}"
        ...
        @can('delete_employees')
            <a class="modal-effect btn btn-sm btn-teacher-archive delete-item" href="#"
               data-id="{{ $row->id }}" data-url="{{ route('admin.Users.employees.destroy', $row->id) }}"
               data-name="{{ $row->name }}">
