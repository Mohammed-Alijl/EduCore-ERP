...
@section('title', trans('admin.Students.promotions.history_title'))
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex align-items-center">
                ...
                <div>
                    <h4 class="content-title mb-0 my-auto font-weight-bold">{{ trans('admin.Students.promotions.history_title') }}
                    </h4>
                    <span class="text-muted mt-1 tx-13 d-block">{{ trans('admin.sidebar.users') }} /
                        {{ trans('admin.Students.promotions.title') }}</span>
                </div>
            </div>
        </div>
        <div class="d-flex my-xl-auto right-content align-items-center">
            <div class="mb-3 mb-xl-0 ml-2">
                <a class="btn btn-primary btn-modern shadow-sm" href="{{ route('admin.Users.students.promotions.index') }}">
                    <i class="las la-arrow-left tx-16 mr-1 ml-1"></i>
                    {{ trans('admin.Students.promotions.back_to_promotions') }}
                </a>
            </div>
        </div>
    </div>
                ...
                <div class="card-header pb-0 border-bottom">
                    <div class="d-flex justify-content-between align-items-center">
                        <h6 class="card-title mb-0">
                            <i class="fas fa-list-alt mr-2"></i>{{ trans('admin.Students.promotions.history_list') }}
                        </h6>
                        <span class="badge badge-primary badge-pill">
                            <i class="fas fa-database mr-1"></i>{{ trans('admin.Students.promotions.all_records') }}
                        </span>
                    </div>
                </div>
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th class="wd-15p border-bottom-0 text-center">
                                        {{ trans('admin.Students.promotions.from_details') }}</th>
                                    <th class="wd-15p border-bottom-0 text-center">
                                        {{ trans('admin.Students.promotions.to_details') }}</th>
                                    <th class="wd-10p border-bottom-0 text-center">{{ trans('admin.Students.promotions.status') }}
                                    </th>
                                    <th class="wd-10p border-bottom-0">{{ trans('admin.Students.promotions.admin') }}</th>
                                    <th class="wd-10p border-bottom-0">{{ trans('admin.Students.promotions.date') }}</th>
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $('#promotion_history_table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('admin.Users.students.promotions.history') }}",
                    type: "GET",
                },
                columns: [{
                        data: 'DT_RowIndex',
            ...
            $(document).on('click', '.btn-rollback', function(e) {
                ...
                Swal.fire({
                    title: '{{ trans('admin.Students.promotions.rollback_confirm_title') }}',
                    html: '<p class="text-muted mb-0">{{ trans('admin.Students.promotions.rollback_confirm_message') }}</p>',
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#dc3545',
                    cancelButtonColor: '#6c757d',
                    confirmButtonText: '<i class="fas fa-undo mr-1"></i> {{ trans('admin.Students.promotions.rollback_btn') }}',
                        ...
                        $.ajax({
                            ...
                            error: function(xhr) {
                                ...
                                const message = (response && response.message) ?
                                    response.message :
                                    '{{ trans('admin.Students.promotions.messages.failed.rollback') }}';
