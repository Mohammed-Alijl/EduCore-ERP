...
@section('title', __('admin.CMS.cms.sections_title'))
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ __('admin.CMS.cms.sections_title') }}</span>
            </div>
        </div>
    </div>
@endsection
                ...
                <div class="cms-header">
                    <div class="cms-header-content">
                        <h2 class="cms-header-title">
                            ...
                            {{ __('admin.CMS.cms.sections_title') }}
                        </h2>
                        <p class="cms-header-subtitle">{{ __('admin.CMS.cms.sections_subtitle') }}</p>
                    </div>
                ...
                <div class="cms-body">
                    @if($sections->isEmpty())
                        <div class="text-center py-5">
                            ...
                            <p class="mt-3 text-muted">{{ __('admin.CMS.cms.no_sections') }}</p>
                        </div>
                    @else
                                ...
                                <div class="cms-section-card {{ !$section->is_visible ? 'cms-section-hidden' : '' }}"
                                    ...
                                    <div class="cms-section-info">
                                        <h5 class="cms-section-name">
                                            {{ __('admin.CMS.cms.sections.' . $section->section_key) }}
                                        </h5>
                                        <p class="cms-section-desc">
                                            {{ __('admin.CMS.cms.section_descriptions.' . $section->section_key) }}
                                        </p>
                                    </div>
                                    <div class="cms-section-actions">
                                        <div class="cms-visibility-toggle">
                                            <label class="cms-switch" title="{{ $section->is_visible ? __('admin.CMS.cms.visible') : __('admin.CMS.cms.hidden') }}">
                                                <input type="checkbox"
                                        ...
                                        @can('edit_cms')
                                            <a href="{{ route('admin.CMS.cms.sections.edit', $section) }}"
        ...
        $(document).ready(function() {
            ...
            const sortable = new Sortable(document.getElementById('sections-sortable'), {
                ...
                onEnd: function() {
                    ...
                    $.ajax({
                        url: "{{ route('admin.CMS.cms.reorder') }}",
                        method: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            order: order
                        ...
                        error: function() {
                            swal("{{ __('admin.global.error') }}", "{{ __('admin.CMS.cms.messages.failed.reorder') }}", "error");
                        }
                    });
                }
            });
            ...
            $(document).on('change', '.toggle-visibility', function() {
                ...
                $.ajax({
                    ...
                    error: function() {
                        ...
                        swal("{{ __('admin.global.error') }}", "{{ __('admin.CMS.cms.messages.failed.toggle') }}", "error");
                    }
                });
            });
        });
