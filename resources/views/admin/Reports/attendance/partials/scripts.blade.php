...
<script>
    $(function() {
        ...
        const absencesDayChart = new ApexCharts(document.querySelector('#absences-by-day-chart'), {
            series: [{
                name: @json(trans('admin.Reports.reports.attendance.charts.absences_count')),
                data: dayData.values
            }],
            chart: {
                type: 'bar',
            ...
            yaxis: {
                title: {
                    text: @json(trans('admin.Reports.reports.attendance.charts.absences_count')),
                    style: {
                        fontWeight: 700
                    }
                }
        ...
        const attendanceTable = $('#attendance_report_table').DataTable(getTableConfig({
            ...
            serverSide: true,
            ajax: {
                url: '{{ route('admin.Reports.reports.attendance.index') }}',
                data: function(d) {
        ...
        $('#attendance-filter-form').on('submit', function(e) {
            ...
            const url = '{{ route('admin.Reports.reports.attendance.index') }}' + (queryString ? '?' +
                queryString : '');
        ...
        $('#btn-reset-filters').on('click', function() {
            window.location.href = '{{ route('admin.Reports.reports.attendance.index') }}';
        });
        ...
        @can('export_attendance-reports')
            ...
            function loadGrades() {
                $.ajax({
                    url: '{{ route('admin.Academic.grades.index') }}',
                    type: 'GET',
                    dataType: 'json',
            ...
            $exportGrade.on('change', function() {
                ...
                $.ajax({
                    url: '{{ route('admin.Academic.sections.get_sections_by_grade') }}',
                    type: 'GET',
                    data: {
                        grade_id: gradeId
                    },
