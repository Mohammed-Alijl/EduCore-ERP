    ...
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content desig-show-modal-content">
            ...
            <div class="modal-body p-4">
                ...
                <!-- Description -->
                <div class="mb-4">
                    <h6 class="font-weight-bold tx-14 mb-2">{{ __('admin.designations.fields.description') }}</h6>
                ...
                <div class="row mb-3">
                    <div class="col-md-4 mb-3">
                        <div class="desig-stat-card">
                            ...
                            <div class="desig-stat-label">{{ __('admin.designations.fields.employees_count') }}</div>
                        </div>
                    </div>
                    <div class="col-md-4 mb-3">
                        <div class="desig-detail-item h-100 d-flex flex-column justify-content-center">
                            <div class="desig-detail-label">{{ __('admin.designations.fields.default_salary') }}</div>
                            <div class="desig-detail-value" id="show-desig-salary">--</div>
                        </div>
                    </div>
                    <div class="col-md-4 mb-3">
                        <div class="desig-detail-item h-100 d-flex flex-column justify-content-center">
                            <div class="desig-detail-label">{{ __('admin.designations.fields.department') }}</div>
...
<script>
    $(document).on('click', '.desig-show-btn', function () {
        ...
        $('#show-desig-description').text(btn.data('description') || '{{ __("admin.designations.no_description") }}');
        $('#show-desig-employees-count').text(btn.data('employees_count') || 0);

        var salary = btn.data('default_salary');
        $('#show-desig-salary').text(salary ? parseFloat(salary).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) : '{{ __("admin.designations.no_salary") }}');

        var teachBadge = $('#show-desig-teach-badge');
        if (btn.data('can_teach') == 1) {
            teachBadge.removeClass('badge-danger').addClass('badge-success').text('{{ __("admin.designations.can_teach_yes") }}');
        } else {
            teachBadge.removeClass('badge-success').addClass('badge-danger').text('{{ __("admin.designations.can_teach_no") }}');
        }
    });
</script>
@endpush
