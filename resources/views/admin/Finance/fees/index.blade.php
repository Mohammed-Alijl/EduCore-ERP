                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th class="wd-25p border-bottom-0">
                                        {{ __('admin.Finance.fees.fields.academic_target') }}</th>
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $('#fees_table').DataTable({
                ...
                ajax: {
                    url: "{{ route('admin.Finance.fees.datatable') }}",
                    data: function(d) {
            ...
            $('select[name="grade_id"]').on('change', function() {
                ...
                if (grade_id) {
                    $.ajax({
                        url: "{{ route('admin.Academic.classrooms.get_classrooms') }}",
                        type: "GET",
                        data: {
                            grade_id: grade_id
                        },
                        success: function(response) {
                            if (response.success) {
                                ...
                                $.each(response.data, function(key, value) {
                                    classroomSelect.append('<option value="' + value
            ...
            $(document).on('click', '.edit-btn', function() {
                ...
                modal.find('select[name="fee_category_id"]').val($(this).data('fee_category_id')).trigger(
                    'change');
                modal.find('select[name="academic_year_id"]').val($(this).data('academic_year_id')).trigger(
                    'change');
