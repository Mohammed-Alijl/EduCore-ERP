...
@section('title', __('admin.Academic.academic_years.title'))
...
@section('css')
    ...
    <link href="{{ URL::asset('assets/admin/css/Academic/academic_year/academic-year-crud.css') }}" rel="stylesheet">
@endsection
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ __('admin.Academic.academic_years.title') }}</span>
            </div>
        </div>
            ...
            @can('create_years')
                <div class="mb-3 mb-xl-0 ml-2">
                    <a class="modal-effect btn btn-primary btn-modern shadow-sm" data-effect="effect-scale" data-toggle="modal"
                        ...
                        {{ __('admin.Academic.academic_years.add') }}
                    </a>
                </div>
            @endcan
        </div>
...
@section('content')
            ...
            <div class="card year-hero-card mb-4">
                <div class="card-body">
                    <div class="d-flex align-items-center justify-content-between flex-wrap">
                        <div>
                            <h5 class="mb-1">{{ __('admin.Academic.academic_years.subtitle') }}</h5>
                            <p class="mb-0 text-muted">{{ __('admin.Academic.academic_years.description') }}</p>
                        </div>
                        <span class="badge badge-pill badge-primary year-count-badge">
                            {{ __('admin.Academic.academic_years.count', ['count' => $academicYears->count()]) }}
                        </span>
                    </div>
                </div>
            </div>
                        ...
                        <table class="table table-hover text-md-nowrap" id="academic_years_table">
                            <thead>
                                <tr>
                                    ...
                                    <th class="wd-25p border-bottom-0">{{ __('admin.Academic.academic_years.fields.name') }}</th>
                                    <th class="wd-20p border-bottom-0">{{ __('admin.Academic.academic_years.fields.starts_at') }}
                                    </th>
                                    <th class="wd-20p border-bottom-0">{{ __('admin.Academic.academic_years.fields.ends_at') }}</th>
                                    <th class="wd-15p border-bottom-0 text-center">
                                        {{ __('admin.Academic.academic_years.fields.is_current') }}</th>
                                ...
                                @foreach ($academicYears as $index => $year)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            @if ($year->is_current)
                                                <span
                                                    class="badge badge-success year-current-badge">{{ __('admin.Academic.academic_years.current') }}</span>
                                            @else
                                                <span
                                                    class="badge badge-secondary year-inactive-badge">{{ __('admin.Academic.academic_years.not_current') }}</span>
                                            @endif
                                        </td>
                                        <td class="text-center">
                                            @include('admin.Academic.academic_years.partials.index_actions', [
                                                'year' => $year,
                                            ])
                                        </td>
                                    </tr>
    ...
    @include('admin.Academic.academic_years.add_modal')
    @include('admin.Academic.academic_years.edit_modal')
    @include('admin.Academic.academic_years.show_modal')
@endsection
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.show-academic-year-btn', function() {
                ...
                $('#show_year_current').text(btn.data('is_current') == 1 ?
                    '{{ __('admin.Academic.academic_years.current') }}' :
                    '{{ __('admin.Academic.academic_years.not_current') }}');
